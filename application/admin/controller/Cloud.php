...
class Cloud extends Admin
{
	...
	{
        $this->redirect("/Admin/Cloud/theme.html");
        exit;
        //$this->checkUpdata();
        return $this->fetch();
	}
    ...
    public function theme()
    {
        ...
        $DbFields = Db::name('Config')->find();
        $DbFields = (array_keys($DbFields));
        if (!in_array('index_html', $DbFields)) {
            Db::execute('ALTER TABLE `tw_config` ADD COLUMN `index_html` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
    ...
    public function themeUp($theme = NULL)
    {
        ...
        if (Db::name('Config')->where('id', 1)->update(array('index_html' => $theme))) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
        }
	...
	{
		if (!Db::name("menu")->where("url", "Cloud/update")->find()) {
			Db::execute("INSERT INTO `tw_menu` (`title`, `pid`, `sort`, `url`,`group`,`ico_name`) VALUES ('自动升级', '10', '1', 'Cloud/update','扩展管理','time')");
		}

		if (!Db::name("menu")->where("url", "Cloud/jiancha")->find()) {
			Db::execute("INSERT INTO `tw_menu` (`title`, `pid`, `sort`, `url`,`group`,`ico_name`) VALUES ('安全检查', '10', '2', 'Cloud/jiancha','扩展管理','time')");
		}
	}
	...
	public function getFileList()
	{
		...
		$authorization = Model("Version")->check_authorization();
		if (strtotime($authorization["shouhou"]) < time()) {
			$this->error("升级失败，你的售后已经到期请联系续费。");
		}

		$aVersion = input("get.version");
		if ($aVersion == "") {
	...
	public function cover()
	{
		...
		if (!file_exists($backup_path)) {
			$this->write(lang("_BACKUP_CREATE_FAIL_PARAM_", array("file" => $backup_path, "file_cloud" => C("CLOUD_PATH"))) . lang("_PERIOD_"), "danger");
			exit();
		} else {
	...
	public function updating5()
	{
		...
		switch ($version["goods"]["entity"]) {
			...
			case 2:
				$moduleModel = Model("Common/Module");
	...
	private function installModule($version, $token)
	{
		...
		if ($res === true) {
			...
			$this->write("&nbsp;&nbsp;&nbsp;>" . lang("_SUCCESS_NAV_CLEAR_") . lang("_PERIOD_"), "success");
