...
class Tools extends Admin
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","cache","delcahe","getDirSize","queue","qianbao","delcache","invoke","rmdirr","excel","export","import");
	...
	{
		$this->redirect('/Admin/Tools/database?type=import');
	}
	...
	public function excel($tables = NULL)
	{
		if ($tables) {
			...
			$zd = Db::table($tables)->getDbFields();
	...
	public function qianbao($id = NULL)
	{
		...
		if (!$qb_list) {
			$qb_list = Db::name('Coin')->where(array('type' => 'qbb'))->select();
			cache($s_key, $qb_list);
		}
		...
		$update_str = '&nbsp;&nbsp;&nbsp;<a href="' . url('Config/coinEdit', array('id' => $qb_list[$id]['id'])) . '" color="green" target="_black">立即前往修改<a>';
