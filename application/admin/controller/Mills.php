...
class Mills extends Admin
{
	...
	public function index($p = 1, $r = 15, $str_addtime = '', $end_addtime = '', $order = '', $status = '', $type = '', $field = '', $name = ''){
		...
		$data = Db::name('Mill')->where($map)->order($order_set)->page($p, $r)->select();
		$count = Db::name('Mill')->where($map)->count();
		$parameter['p'] = $p;
		...
		$builder->title('矿机产品');
		$builder->titleList('产品列表', url('Mills/index'));
		$builder->button('add', '添 加', url('Mills/add'));
		$builder->button('delete', '删 除', url('Mills/status', array('model' => 'Mill', 'status' => -1)));
		$builder->setSearchPostUrl(url('Mills/index'));
	...
	public function add( $id = NULL ){
		if($this->request->ispost()){
			...
			if (check($_POST['id'], 'd')) {
				$rs = Db::name('Mill')->update($_POST);
			}
			else {
				$rs = Db::name('Mill')->insert($_POST);
			}
			...
			$builder->title('矿机管理');
			$builder->titleList('矿机列表', url('Mills/index'));

			if ($id) {
				$builder->keyReadOnly('id', '类型id');
				$builder->keyHidden('id', '类型id');
				$data = Db::name('Mill')->where('id', $id)->find();
			...
			$builder->keyImage('images', '矿机图片', '矿机图片', array('width' => 408, 'height' => 300, 'savePath' => 'shop', 'url' => url('Shop/images')));
			...
			$builder->keyAddTime();
			$builder->keyStatus('status','状态','',array('开放购买','未开放购买','已售罄'));
			$builder->savePostUrl(url('Mills/add'));
			$builder->display();
		}
	}
	...
	public function payment($p = 1, $r = 15,  $level = '',$starttime='',$endtime='', $username = ''){
		...
		$data = Db::name('MillLog')->where($map)->order('id desc')->page($p, $r)->select();
		$count = Db::name('MillLog')->where($map)->count();
		...
		$builder = new BuilderList();
		$builder->title('交易记录');
		$builder->setSearchPostUrl(url('Mills/payment'));
		$mill_first[0] = '请选择矿机';
	...
	public function deleteMill($id){
		if( Db::name('MillLog')->where('id', $id)->update(array('status'=>2)) ){
			$this->success('操作成功');
		}else{
			$this->error('操作失败');
		}
	...
	public function fenhong($p = 1, $r = 15, $order = '', $username = ''){
		...
		$data = Db::name('MillFenhong')->where($map)->order($order_set)->page($p, $r)->select();

		$count = Db::name('MillFenhong')->where($map)->count();
		$parameter['p'] = $p;
		$parameter['order'] = $order;
		$parameter['username'] = $username;
		$builder = new BuilderList();
		$builder->title('分红管理');
		$builder->titleList('发送列表', url('Mills/fenhong'));
		$builder->button('add', '发送分红', url('Mills/add_fenhong'));
		$builder->setSearchPostUrl(url('Mills/fenhong'));
		$builder->search('order', 'select', array('id_desc' => 'ID降序', 'id_asc' => 'ID升序'));
		$builder->search('username', 'text', '请输入查询内容');
		$builder->keyId();
		$builder->keyText('username', '用户名');
	...
	public function add_fenhong( $id = NULL ){
		if($this->request->ispost()){
			...
			if( !$mutil_user ){
				$userid = Db::name('User')->where('username', trim($_POST['username']))->field('id')->find();
			...
			if( !$mutil_user ){
				$result = Db::name('userCoin')->where('userid', $userid['id'])->setInc($_POST['type'],$_POST['money']);

				if( !$result ){
					//dump(Db::name('userCoin')->getLastSql());
					$this->error('发送分红失败');
				}else{
					Db::name('MillFenhong')->insert([
								'userid'   => $userid['id'],
								'money'    => $_POST['money'],
								'type' 	   => $_POST['type'],
				...
				foreach ($username as  $value) {
					$userid = Db::name('User')->where('username', trim($value))->field('id')->find();
					if( $userid ){
						$result = Db::name('userCoin')->where('userid', $userid['id'])->setInc($_POST['type'],$_POST['money']);
			...
			$builder = new BuilderEdit();
			$builder->title('发布分红');
			$builder->titleList('分红管理', url('Mills/fenhong'));
			...
			$builder->savePostUrl(url('Mills/add_fenhong'));
			$builder->display();
		}
	}
	...
	public function setting(){
		if( $this->request->ispost() ){
			...
			if( Db::name('millConfig')->where('id', 1)->update($data) ){
				$this->success('保存成功');
			}else{
				$this->error('保存失败');
			}
			...
			//$builder->keyText('mill_'.$key, $value, '%，请使用英文逗号 , 来分割五级分销的利率');
			$config = Db::name('millConfig')->where('id', 1)->find();
			$data  = unserialize($config['config']);
			$builder->data($data);

			$builder->savePostUrl(url('Mills/setting'));
			$builder->display();
		}
	}
	...
	public function fenxiao($p = 1, $r = 15, $starttime='',$endtime='', $username = ''){
		...
		$data = Db::name('MillFenxiao')->where($map)->order('id desc')->page($p, $r)->select();
		$count = Db::name('MillFenxiao')->where($map)->count();
		...
		$builder = new BuilderList();
		$builder->title('分销记录');
		$builder->titleList('分销设置',url('Mills/setting'));
		$builder->setSearchPostUrl(url('Mills/fenxiao'));
		$builder->search('starttime', 'time','开始日期');
		$builder->search('endtime', 'time','结束日期');
