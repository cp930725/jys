...
class BuilderEdit extends Admin
{
	private $_title;
	private $_suggest;
	private $_titleList;
	...
	public function handleConfig()
	{
		if ($this->request->ispost()) {
			...
			foreach (input('') as $k => $v) {
				$config['name'] = '_' . strtoupper($this->request->controller()) . '_' . strtoupper($k);
				$config['type'] = 0;
				$config['title'] = '';
				$config['group'] = 0;
				$config['extra'] = '';
				...
				if ($configModel->insert($config, null, true)) {
					$success = 1;
				}

				$tag = 'conf_' . strtoupper($this->request->controller()) . '_' . strtoupper($k);
				cache($tag, null);
			}
			...
			if ($success) {
				if ($this->_callback) {
					$str = $this->_callback;
					A($this->request->controller())->$str(input(''));
				}
			...
			$configs = Model('Config')->where(array(
				'name' => array('like', '_' . strtoupper($this->request->controller()) . '_' . '%')
				))->limit(999)->select();
			$data = array();

			foreach ($configs as $k => $v) {
				$key = str_replace('_' . strtoupper($this->request->controller()) . '_', '', strtoupper($v['name']));
				$data[$key] = $v['value'];
			}
	...
	public function groupLocalComment($group_name, $mod)
	{
		...
		$this->keyRadio($mod . '_LOCAL_COMMENT_CAN_GUEST', lang('_COMMENTS_ALLOW_VISITOR_IF_'), lang('_ALLOW_DEFAULT_'), array(lang('_DISALLOW_'), lang('_ALLOW_')))->keyRadio($mod . '_LOCAL_COMMENT_ORDER', lang('_COMMENTS_SORT_'), lang('_DESC_DEFAULT_'), array(lang('DESC'), lang('ASC')))->keyText($mod . '_LOCAL_COMMENT_COUNT', lang('_COMMENTS_PAGE_DISPLAY_COUNT_'), lang('post_show_image_number_default'));
