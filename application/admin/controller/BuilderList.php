...
class BuilderList extends AdminController
{
	...
	public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '')
	{
		...
		$this->convertKey('userid', 'text', function($value) {
			...
			if ($username) {
				return '<a href=\'' . url('User/index', array('type' => 'username', 'name' => $username)) . '\' target=\'_blank\'>' . $username . '</a>';
			}
			else {
				return '---';
			}
		...
		$this->convertKey('shopid', 'text', function($value) {
			$name = Model('Shop')->getShopName($value);

			if ($name) {
				return '<a href=\'' . url('Shop/index', array('field' => 'name', 'name' => $name)) . '\' >' . $name . '</a>';
			}
			else {
				return '---';
			}
