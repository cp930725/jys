...
class Ptpbc extends AdminController
{
	...
	public function index($market = NULL, $lang = NULL, $merchant = NULL){
		...
		$show = Db::name('Ptpbc')->where($where)->paginate(10);

		$list = Db::name('Ptpbc')->where($where)->order('id desc')->limit(0, 10)->select();
	...
	public function edit(){
		$data=Db::name('Ptpbc')->where('id', input('id'))->find();
		$this->assign('data',$data);
		$this->display('ptpbcEdit');
	}
	public function del(){
		$data=Db::name('Ptpbc')->where('id', input('id'))->delete();
		$this->success('删除成功');
	}
	public function save_action(){
		...
		if ($data['id']) {
			$rs = Db::name('Ptpbc')->update($data);
		}else {
			$data['addtime'] = time();
			$rs = Db::name('Ptpbc')->insert($data);
		}
		if ($rs) {
			$this->success('编辑成功！');
		}else{
	...
	public function log($field = NULL, $name = NULL, $market = NULL,$status = NULL, $starttime = NULL, $endtime = NULL){
		...
		$list_new = Db::name('PtpbcLog')->where($where)->select();
		...
		$show = Db::name('PtpbcLog')->where($where)->paginate(10);

		$list = Db::name('PtpbcLog')->where($where)->order('id desc')->limit(0, 10)->select();
	...
	public function comment($field = NULL, $name = NULL, $coinname = NULL)
	{
		...
		$show = Db::name('CoinComment')->where($where)->paginate(10);
