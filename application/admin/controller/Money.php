<?php
namespace app\admin\controller;

use think\Db;

class Money extends Admin
{
	private $Model;

	public function __construct()
	{
		parent::__construct();
		$this->Model = Db::name('Money');
	}

	public function index()
	{
		$where = array('status' => array('egt', 0));
		$show = $this->Model->where($where)->paginate(10);

		$list = $this->Model->where($where)->order('id desc')->limit(0, 10)->select();

		foreach ($list as $k => $v) {
			$list[$k]['tian'] = $list[$k]['tian'].' '.$this->danweitostr($list[$k]['danwei']);
		}

		$this->assign('list', $list);
		$this->assign('page', $show);
		return $this->fetch();
	}

	public function edit()
	{
		if (empty($_GET['id'])) {
			$this->data = false;
			$this->assign('data', $this->data);
		} else {
			$data = array();
			$data = $this->Model->where('id', trim($_GET['id']))->find();
			$data['deal'] = Db::name('MoneyLog')->where('money_id', $data['id'])->sum('num');
			$this->data = $data;
            $this->assign('data', $this->data);
        }

		return $this->fetch();
	}

	public function save()
	{
		if (APP_DEMO) {
			$this->error('测试站暂时不能修改！');
		}

		if ($_POST['outfee'] && (($_POST['outfee'] < 0.01) || (500 < $_POST['outfee']))) {
			$this->error('活期取现手续费范围为0.01 -- 500 %！');
		}
		if (floatval($_POST['num']) < floatval($_POST['deal'])) {
			$this->error('理财总量不能小于已理财量');
		}
		if (floatval($_POST['stnum'] <= 0)) {
			$this->error('最低起存数量不能小于0');
		}
		if (!floatval($_POST['tian'])) {
			$this->error('理财周期不能为空');
		}
		if ($_POST['type'] == 2 && !floatval($_POST['jian'])) {
			$this->error('利息释放间隔不能为空');
		}
		if (($_POST['fee'] < 0.01) || (500 < $_POST['fee'])) {
			$this->error('周期利息范围为0.01 -- 500 %！');
		}

		if (strtotime($_POST['addtime']) != strtotime(addtime(strtotime($_POST['addtime'])))) {
			$this->error('开始时间格式错误！');
		}
		if (strtotime($_POST['endtime']) != strtotime(addtime(strtotime($_POST['endtime'])))) {
			$this->error('结束时间格式错误！');
		}
		
		$_POST['addtime'] = strtotime($_POST['addtime']);
		$_POST['endtime'] = strtotime($_POST['endtime']);

		switch ($_POST['danwei']) {
			case 'y':
				$_POST['step'] = $_POST['tian'] * 12 * 30 * 24 * 60 * 60;
				break;

			case 'm':
				$_POST['step'] = $_POST['tian'] * 30 * 24 * 60 * 60;
				break;

			case 'd':
				$_POST['step'] = $_POST['tian'] * 24 * 60 * 60;
				break;

			case 'h':
				$_POST['step'] = $_POST['tian'] * 60 * 60;
				break;

			default:

			case 'i':
				$_POST['step'] = $_POST['tian'] * 60;
				break;
		}
		
		switch ($_POST['jiandanwei']) {
			case 'y':
				$_POST['jianstep'] = $_POST['jian'] * 12 * 30 * 24 * 60 * 60;
				break;

			case 'm':
				$_POST['jianstep'] = $_POST['jian'] * 30 * 24 * 60 * 60;
				break;

			case 'd':
				$_POST['jianstep'] = $_POST['jian'] * 24 * 60 * 60;
				break;

			case 'h':
				$_POST['jianstep'] = $_POST['jian'] * 60 * 60;
				break;

			default:

			case 'i':
				$_POST['jianstep'] = $_POST['jian'] * 60;
				break;
		}
		
		if ($_POST['id']) {
			$rs = $this->Model->where('id', $_POST['id'])->update($_POST);
		} else {
			$rs = $this->Model->insert($_POST);
		}

		if ($rs) {
			$this->success('操作成功！',url('Money/index'));
		} else {
			debug($this->Model->getDbError(), 'lastSql');
			$this->error('操作失败！');
		}
	}

	public function log($money_id = NULL, $name = NULL)
	{
		if ($name && check($name, 'username')) {
			$where['userid'] = Db::name('User')->where('money_id', $money_id)->where('username', $name)->value('id');
		} else {
			$where = array(array('money_id' => $money_id));
		}

		$this->Model = Db::name('MoneyLog');
		$show = $this->Model->where($where)->paginate(10);

		$list = $this->Model->where($where)->order('id desc')->limit(0, 10)->select();

		foreach ($list as $k => $v) {
			$list[$k]['username'] = Db::name('User')->where('id', $v['userid'])->value('username');
			$list[$k]['money'] = Db::name('Money')->where('id', $v['money_id'])->find();
			$list[$k]['money']['tian'] = $list[$k]['money']['tian'] . ' ' . $this->danweitostr($list[$k]['money']['danwei']);
		}

		$this->assign('money_id', $money_id);
		$this->assign('list', $list);
		$this->assign('page', $show);
		return $this->fetch();
	}

	public function fee($userid = NULL, $money_id = NULL)
	{
		$where['userid'] = $userid;
		$user = Model('User')->where('id', $userid)->find();

		if (!$user) {}

		$this->Model = Db::name('MoneyFee');
		$show = $this->Model->where($where)->paginate(10);

		$list = $this->Model->where($where)->order('id desc')->limit(0, 10)->select();

		foreach ($list as $k => $v) {
			$list[$k]['money'] = Db::name('Money')->where('id', $v['money_id'])->find();
		}

		debug($v, 'v');
		$this->assign('money_id', $money_id);
		$this->assign('user', $user);
		$this->assign('list', $list);
		$this->assign('page', $show);
		return $this->fetch();
	}

	public function status()
	{
		if (APP_DEMO) {
			$this->error('测试站暂时不能修改！');
		}

		if ($this->request->ispost()) {
			$id = array();
			$id = implode(',', $_POST['id']);
		} else {
			$id = $_GET['id'];
		}

		if (empty($id)) {
			$this->error('请选择要操作的数据!');
		}

		$where['id'] = array('in', $id);
		$method = $_GET['method'];

		switch (strtolower($method)) {
			case 'forbid':
				$data = array('status' => 0);
				break;

			case 'resume':
				$data = array('status' => 1);
				break;

			case 'delete':
				if ($this->Model->where($where)->delete()) {
					$this->success('操作成功！');
				} else {
					$this->error('操作失败！');
				}

				break;

			default:
				$this->error('参数非法');
		}

		if ($this->Model->where($where)->update($data)) {
			$this->success('操作成功！');
		} else {
			$this->error('操作失败！');
		}
	}

	private function danweitostr($danwei)
	{
		switch ($danwei) {
			case 'y':
				return '年';
				break;

			case 'm':
				return '月';
				break;

			case 'd':
				return '天';
				break;

			case 'h':
				return '小时';
				break;

			default:

			case 'i':
				return '分钟';
				break;
		}
	}
}

?>