...
{
    //设置首页链接
    $pos_str = "<li><a href=".url('/Support').">".L('帮助中心')."</a></li>";

    //如果cid为空，获取串，否则视为首页
    if(!empty($cid))
    {
        //获取当前栏目信息
        $r = Db::name('article_type')->where('id', $cid)->find();
        if (empty($r['id'])) {
            return $pos_str."<li><span>&gt;</span>".L('栏目不存在')."</li>";
        } else {
            //构成上级栏目字符
            if ($r['pid'] != '0') {
                $cvv = Db::name('article_type')->where('id', $cid)->find();
                if ($cvv['pid'] > 0) {
                    $r = Db::name('article_type')->where('id', $cvv['pid'])->find();
                    $pos_str .= "<li><span>&gt;</span><a href=".url('Support/index/categories/cid/'.$cvv['pid']).">".$r['title']."</a></li>";
                }
            }
            ...
            if (isset($r) && is_array($r)) {
                if (!empty($id)) {
                    return $pos_str."<li><span>&gt;</span><a href=".url('Support/index/sections/cid/'.$r['pid'].'/id/'.$r['id']).">".$r['title']."</a></li><li><span>&gt;</span>正文</li>";
                } else {
...
{
    return new \app\common\ext\EthCommon($host, $port);
}
...
function authgame($name)
{
    ...
    if (db('VersionGame')->where('name', $name)->where('status', 1)->find()) {
        return 1;
    } else {
        return 0;
        exit();
...
function huafei($mobile = NULL, $num = NULL, $orderid = NULL)
{
    ...
    $recharge = new \app\common\ext\Recharge($appkey, $openid);
...
{
    if ($username && $type) {
        $userid = (config('app.develop') ? NULL : cache('userid' . $username . $type));

        if (!$userid) {
            $userid = Db::name('User')->where($type, $username)->value('id');
...
{
    if ($id && $type) {
        $username = (config('app.develop') ? NULL : cache('username' . $id . $type));

        if (!$username) {
            $username = Db::name('User')->where($type, $id)->value('username');
...
{
    if (config('app.app_debug`') || MSCODE) {
        if (!PHP_SAPI == 'cli' ? 1 : 0) {
            \app\common\ext\FirePHP::getInstance(true)->log($value, $type);
        }
    }
}
...
{
    return new \app\common\ext\CoinClient($username, $password, $ip, $port, $timeout, $headers, $suppress_errors);
}

function EthClient($username, $password, $ip, $port, $timeout = 3, $headers = array(), $suppress_errors = false)
{
    return new \app\common\ext\EthCommon($username, $password, $ip, $port, $timeout, $headers, $suppress_errors);
}

function coinname($type){
    if( !S('COINNAME') ){
        $coin_list = Db::name('Coin')->get_all_name_list();
        cache('COINNAME',$coin_list,3600);
    }
    $coinname = cache('COINNAME');
...
function get_city_ip($ip = NULL)
{
    ...
    $Ip = new \xin\iplocation\IpLocation();
    $area = $Ip->getLocation();
...
function get_user($id, $type = NULL, $field = 'id')
{
    ...
    if (!$data) {
        $data = Db::name('User')->where($field, $id)->find();
        cache($key, $data);
    }
...
function send_mobiles($mobile, $content)
{
    ...
    $url = config('mobile_url') . '/?Uid=' . config('mobile_user') . '&Key=' . config('mobile_pwd') . '&smsMob=' . $mobile . '&smsText=' . $content;
...
function get_link($link_id = NULL, $field = 'url')
{
    ...
    $link = Db::name('Url')->getById($link_id);
...
{
    $user = session(cache('USER_AUTH_KEY'));
...
function is_login()
{
    ...
    if (empty($user)) {
        ...
        return session(cache('USER_AUTH_SIGN_KEY')) == user_auth_sign($user) ? $user['admin_id'] : 0;
    }
}
...
function get_model_attribute($model_id, $group = true)
{
    ...
    if (!isset($list[$model_id])) {
        ...
        $info = Db::name('Attribute')->where($map)->select();
        $list[$model_id] = $info;
    }
    ...
    if ($group) {
        $sort = Db::name('Model')->getFieldById($model_id, 'field_sort');
...
{
    $tags = Db::name('Article')->getFieldById($id, 'tags');
...
function sendTelCode($tel, $data, $tempId, $appid)
{
    ...
    $account_sid = '8a216da86b652116016b685f603f0124';
    $version = '2013-12-26';
    $auth_token = 'deb9c9317fa14189928f03fda87658d0';
    $batch = date('YmdHis');
    $body = "{'to':'$tel','templateId':'$tempId','appId':$appid,'datas':[" . $option . "]}";
    $sig = strtoupper(md5($account_sid . $auth_token . $batch));
    $url = $rest_url . '/' . $version . '/Accounts/' . $account_sid . '/SMS/TemplateSMS?sig=' . $sig;
    $authen = base64_encode($account_sid . ':' . $batch);
    $header = [
