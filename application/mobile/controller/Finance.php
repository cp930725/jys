...
class Finance extends Mobile
{
	...
	public function index()
	{
		...
		empty($cny['dj']) ? '' : $cny['dj'] = sprintf("%.2f", $cny['dj']);
        empty($cny['ky']) ? '' : $cny['ky'] = sprintf("%.2f", $cny['ky']);
        empty($cny['zj']) ? '' : $cny['zj'] = sprintf("%.2f", $cny['zj']);
        empty($cny['dj']) ? '' : $cny['dj'] = number_format($cny['dj'],2);//千分位显示
        empty($cny['ky']) ? '' : $cny['ky'] = number_format($cny['ky'],2);//千分位显示
	...
	public function coin_show($coin)
	{
		...
		$Coin = Db::name('Coin')->where(array('id' => $coin,'status' => 1))->find();
		$CoinInfo = $Coin;
		...
		if (!empty($Market[$Coin['name'].'_'.config('app.anchor_cny')]['new_price'])) {
			$jia = $Market[$Coin['name'].'_'.config('app.anchor_cny')]['new_price'];
		} else {
			$jia = 1;
		}
		...
		$user_coin['xnbd'] = empty($user_coin[$Coin['name'].'d']) ? 0 : sprintf("%.6f", $user_coin[$Coin['name'].'d']);
		$user_coin['zhehe'] =  empty($user_coin[$Coin['name']]) && empty($user_coin[$Coin['name'] . 'd']) ? 0 : round(($user_coin[$Coin['name']] + $user_coin[$Coin['name'] . 'd']) * $jia, 2);
	...
	public function mytj()
	{
		...
		$useracc= Db::name('User')->where(array('id' => $user['invit_1']))->value('username');
		if (!$user['invit']) {
			for (; true; ) {
				$tradeno = tradenoa();
			...
			Db::name('User')->where(array('id' => userid()))->save(array('invit' => $tradeno));
			$user = Db::name('User')->where(array('id' => userid()))->find();
		}
		$user_url=$this->getShort("http://".$_SERVER['HTTP_HOST']."/home/Login/register/invit/".$user['invit']);
	...
	public function mywd()
	{
		...
		$show = $Page->show();
		$list = $Model->where($where)->order('id asc')->field('id,username,mobile,addtime,invit_1')->limit(0, 10)->select();

		foreach ($list as $k => $v) {
			$list[$k]['invits'] = Db::name('User')->where(array('invit_1' => $v['id']))->order('id asc')->field('id,username,mobile,addtime,invit_1')->select();
			$list[$k]['invitss'] = count($list[$k]['invits']);

			foreach ($list[$k]['invits'] as $kk => $vv) {
				$list[$k]['invits'][$kk]['invits'] = Db::name('User')->where(array('invit_1' => $vv['id']))->order('id asc')->field('id,username,mobile,addtime,invit_1')->select();
	...
	public function myzr_log($coin = null)
	{
		...
		$show = $Page->show();
		$list = $Moble->where($where)->order('id desc')->limit(0, 10)->select();
	...
	public function mycj_coin_list()
	{
		...
		// $map['status'] = 1;
		// $coin_list = Db::name('Coin')->where($map)->order('id desc')->select();

		$coin_list = Db::name('market')->where('status=1')->select();
		foreach ($coin_list as $key => $v) {
			$xnb = explode('_', $v['name'])[0];
			$rmb = explode('_', $v['name'])[1];
			$coinxx = Db::name('coin')->where(array('name'=>$xnb))->find();
	...
	*/
	public function myzcadd($coin = NULL)
	{
		...
		if (!$coin_list[$coin]['zc_jz']) {
			...
			$moble = Db::name('User')->where(array('id' => userid()))->value('mobile');

			if ($moble) {
				$moble = substr_replace($moble, '****', 3, 4);
			} else {

				$this->redirect(url('/User/mobile'));
				exit();
			}
	...
	public function mycz($status = NULL)
	{
		...
		$myczType = Db::name('MyczType')->where(array('status' => 1))->select();
		...
		$user_info = Db::name('user')->where(array('id'=>userid()))->find();
	...
	public function myczHuikuan($id = NULL)
	{
		...
		$rs = Db::name('Mycz')->where(array('id' => $id))->save(array('status' => 3));
		if ($rs) {
	...
	public function myczFee($cztype)
	{
		...
		$cztype_list = Db::name('mycz_type')->where(array('status'=>1))->select();
		$cztype_arr=array();
		...
		$fee = Db::name('mycz_type')->where(array('status'=>1,'name'=>$cztype))->find();
	...
	public function myczUp($bankt = '', $type, $num, $mum, $truename, $aliaccount)
	{
		...
		if ($num < $mycz_min || $mum < $mycz_min) {
			$this->error(lang('充值金额不能小于') . $mycz_min . L('元！'));
		}
		if ($mycz_max < $num || $mycz_max < $mum) {
			$this->error(lang('充值金额不能大于') . $mycz_max . L('元！'));
		}
		...
		$mycz = Db::name('Mycz')->insert(array('userid' => userid(), 'bank' => $bankt, 'num' => $num, 'mum' => $mum, 'type' => $type, 'tradeno' => $tradeno, 'addtime' => time(), 'status' => 0, 'alipay_truename'=>$truename, 'alipay_account'=>$aliaccount, 'fee'=>$myczType['fee']));
		if ($mycz) {
			if ($type!='weixin') {
				$this->success(lang('充值订单创建成功！'), array('id' => $mycz));
			} elseif($type=='weixin') {
				...
				if (!empty($wxpay['code_url'])) {
					...
					Db::name('Mycz')->where(array('id'=>$mycz))->save(array('ewmname'=>$imgname));
					$res=array();
					$res['cztype']="wxpay";
					$res['status']=1;
					$res['id']=$mycz;
	...
	public function mytx($status = NULL)
	{
		...
		$mobile = Db::name('User')->where(array('id' => userid()))->value('mobile');
		if ($mobile) {
		...
		$truenames = Db::name('User')->where(array('id' => userid()))->value('truename');
	...
	public function mytxUp($mobile_verify, $num, $paypassword, $type)
	{
		...
		$mytx_min = (config('mytx_min') ? C('mytx_min') : 2);
		$mytx_max = (config('mytx_max') ? C('mytx_max') : 50000);

		$mytx_day_max = (config('mytx_day_max') ? C('mytx_day_max') : 200000);
		$start_time = mktime(0,0,0,date('m'),date('d'),date('Y'));
		$end_time = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
		$today_tx_sum = Db::name('finance_log')->where(array('addtime'=>array('between',"$start_time,$end_time"),'optype'=>5,'userid' => session('userId')))->field('sum(amount) as ttamount')->find();
		...
		$mytx_fee_min = (config('mytx_fee_min') ? C('mytx_fee_min') : 0);
		if($mytx_min<=$mytx_fee_min){
			$mytx_min=$mytx_fee_min;
		}
		if ($num < $mytx_min) {
			$this->error(lang('每次提现金额不能小于') . $mytx_min . L('元！'));
		}
		if ($mytx_max < $num) {
			$this->error(lang('每次提现金额不能大于') . $mytx_max . L('元！'));
		}
		if ($mytx_bei) {
			if ($num % $mytx_bei != 0) {
				$this->error(lang('每次提现金额必须是') . $mytx_bei . L('的整倍数！'));
			}
		}
		...
		try{
			...
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
			...
			$rs[] = Db::table('tw_finance_log')->insert(array('username' => session('userName'), 'adminname' => session('userName'), 'addtime' => time(), 'plusminus' => 0, 'amount' => $num, 'optype' => 5, 'position' => 1, 'cointype' => 1, 'old_amount' => $finance_num_user_coin['cny'], 'new_amount' => $finance_mum_user_coin['cny'], 'userid' => session('userId'), 'adminid' => session('userId'),'addip'=>get_client_ip()));
			// 处理资金变更日志-----------------E
	...
	public function mytxChexiao($id)
	{
		...
		$rs = array();
		$finance = Db::table('tw_finance')->where(array('userid' => $mytx['userid']))->order('id desc')->find();
		$finance_num_user_coin = Db::table('tw_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$rs[] = Db::table('tw_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
		$rs[] = Db::table('tw_mytx')->where(array('id' => $mytx['id']))->setField('status', 2);
		$finance_mum_user_coin = Db::table('tw_user_coin')->where(array('userid' => $mytx['userid']))->find();
		...
		// 处理资金变更日志-----------------S
		$rs[] = Db::table('tw_finance_log')->insert(array('username' => session('userName'), 'adminname' => session('userName'), 'addtime' => time(), 'plusminus' => 1, 'amount' => $mytx['num'], 'optype' => 24, 'position' => 1, 'cointype' => 1, 'old_amount' => $finance_num_user_coin['cny'], 'new_amount' => $finance_mum_user_coin['cny'], 'userid' => session('userId'), 'adminid' => session('userId'),'addip'=>get_client_ip()));
		// 处理资金变更日志-----------------E
...
/*		if (config('coin')[$coin]) {
		...
		$Coins = Db::name('Coin')->where(array(
			'status' => 1,
			'type'   => array('neq', 'ptb'),
			'name'   => array('neq', config('app.anchor_cny')),
		))->select();
		...
		$user_coin[$coin.'d'] = empty($user_coin[$coin.'d']) ? 0 : round($user_coin[$coin.'d'], 6);
		$user_coin[$coin.'d'] = empty($user_coin[$coin.'d']) ? 0 : sprintf("%.4f", $user_coin[$coin.'d']);
		
		$this->assign('xnb_c', $user_coin[$coin]);
		$this->assign('xnbd_c', $user_coin[$coin.'d']);
		$this->assign('user_coin', $user_coin);
		
		$Coins = Db::name('Coin')->where(array('name' => $coin))->find();
		...
		if (!$Coins['zr_jz']) {
			...
			if (empty($user_coin[$qbdz])) {
				if ($Coins['type'] == 'rgb') {
					...
					$rs = Db::name('UserCoin')->where(array('userid' => userid()))->save(array($qbdz => $qianbao));
					if (!$rs) {
				...
				if ($Coins['type'] == 'qbb') {
					...
					$coin_config = Db::name('Coin')->where(array('name' => $coin))->find();

					if ($coin=='eth' || $coin=='eos' || $coin_config['token_type'] == 1)  //ETH对接,FFF
					{
						$coin_select = Db::name('Coin')->where(array('api_type' => 'eth','token_type' => 1))->select();
						...
						if (!$qianbao) {
							...
							if (!$EthClient) {
								...
								if (!$qianbao || preg_match("/[\'.,:;*?~`!@#$%^&+=)(<>{}]|\]|\[|\/|\\\|\"|\|/",$qianbao)) {
									...
									foreach ($ethcoin as $k => $v) {
									$rs = Db::name('UserCoin')->where(array('userid' => userid()))->save(array($v.'b' => $qianbao));
								}
							}
						}
					...
					if (!$qianbao || preg_match("/[\'.,:;*?~`!@#$%^&+=)(<>{}]|\]|\[|\/|\\\|\"|\|/",$qianbao)) {
						...
						$rs = Db::name('UserCoin')->where(array('userid' => userid()))->save(array('etcb' => $qianbao));
						// $rs = Db::name('UserCoin')->where(array('userid' => userid()))->save(array('tatcb' => $qianbao));
					}
		...
		$this->assign('qianbao', empty($qianbao) ? null : $qianbao);
		$where['userid'] = userid();
		$where['coinname'] = $coin;
		$where['from_user'] = '0';
	...
	public function myzrold($coin = NULL)
	{
		...
		// 获取币种信息
		$coin_info = Db::name('Coin')->where(array('name' => $coin))->find();
		if(!$coin_info){
	...
	public function myzc($coin = NULL)
	{
		...
		$Mobile = Db::name('Myzc');
		$show = $Mobile->where($where)->paginate();
	...
	public function myzc_user($coin = NULL,$jf_type =NULL)
	{
		...
		if ($jf_type == 'jf_zr') {
			...
			$list = $Mobile->where($where)->order('id desc')->limit(0, 10)->select();
			// foreach ($list as $k => $v) {
			// 	// $users_n = Db::name('User')->where(array('id' => $v['userid']))->value('username');
...
/*		if ($user_info['mobile'] != session('chkmobile')) {
		...
		if ($coin=='tatc') {
			if ($num <100) {
				$this->error(lang('数量不能低于100！'));
			}
		} else {
			if ($num <0.1) {
				$this->error(lang('数量不能低于0.1！'));
			}
		}
		...
		$qbdz = $coin . 'b';
		$fee_user = Db::name('UserCoin')->where(array($qbdz => $Coins['zc_user']))->find();
		if ($fee_user) {
			debug(lang('手续费地址: ') . $Coins['zc_user'] . L('存在,有手续费'));
			...
			debug(lang('手续费地址: ') . $Coins['zc_user'] . L('不存在,无手续费'));
			$fee = 0;
			$mum = $num;
		}
		...
		if ($Coins['type'] == 'rgb') { //认购币
			...
			$rs = array();
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);
			if ($fee) {
				if (Db::table('tw_user_coin')->where(array($qbdz => $Coins['zc_user']))->find()) {
					$rs[] = Db::table('tw_user_coin')->where(array($qbdz => $Coins['zc_user']))->setInc($coin, $fee);
					debug(array('msg' => L('转出收取手续费') . $fee), 'fee');
				} else {
					$rs[] = Db::table('tw_user_coin')->insert(array($qbdz => $Coins['zc_user'], $coin => $fee));
					debug(array('msg' => L('转出收取手续费') . $fee), 'fee');
				}
			}
			...
			// 转出人记录
			$user_zj_coin = Db::table('tw_user_coin')->where(array('userid' => userid()))->find();
			$rs[] = Db::table('tw_finance_log')->insert(array('username' => $user['username'], 'adminname' => $user['username'], 'addtime' => time(), 'plusminus' => 0, 'amount' => $num, 'optype' => 6, 'position' => 1, 'cointype' => $Coins['id'], 'old_amount' => $user_coin[$coin], 'new_amount' => $user_zj_coin[$coin], 'userid' => userid(), 'adminid' => userid(),'addip'=>get_client_ip()));

			// 获取用户信息
			$user_info = Db::table('tw_user')->where(array('id' => $peer['userid']))->find();
			$user_peer_coin = Db::table('tw_user_coin')->where(array('userid' => $peer['userid']))->find();

			// 接受人记录
			$rs[] = Db::table('tw_finance_log')->insert(array('username' => $user_info['username'], 'adminname' => $user['username'], 'addtime' => time(), 'plusminus' => 1, 'amount' => $mum, 'optype' => 7, 'position' => 1, 'cointype' => $Coins['id'], 'old_amount' => $peer[$coin], 'new_amount' => $user_peer_coin[$coin], 'userid' => $peer['userid'], 'adminid' => userid(),'addip'=>get_client_ip()));
		...
		if ($Coins['type'] == 'qbb') { //钱包币
			$mo = Db::name();
			if (Db::table('tw_user_coin')->where(array($qbdz => $addr))->find()) {
...
/*					 if ($json['balance'] < $num) {
				...
				try{
					...
					$rs = array();
					$rs[] = $r = Db::table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
					...
					if ($fee && $auto_status) {
						...
						if (Db::table('tw_user_coin')->where(array($qbdz => $Coins['zc_user']))->find()) {
							$rs[] = $r = Db::table('tw_user_coin')->where(array($qbdz => $Coins['zc_user']))->setInc($coin, $fee);
							debug(array('res' => $r, 'lastsql' => Db::table('tw_user_coin')->getLastSql()), '新增费用');
						} else {
							$rs[] = $r = Db::table('tw_user_coin')->insert(array($qbdz => $Coins['zc_user'], $coin => $fee));
						}
					}
...
/*								Db::execute('commit');
							...
							if ($sendrs) {
								$res = Db::table('tw_myzc')->where(array('id'=>$aid))->save(array('txid'=>$sendrs));
								Db::execute('commit');
								Db::execute('unlock tables');
							} else {
...
/*		if (!check($mobile_verify, 'd')) {
		...
		$addr_user = Db::name('User')->where(array('username' => $addr))->find();
		$from_user = Db::name('User')->where(array('id' => userid()))->find();
		if (!$addr_user) {
			$this->error(lang('转入用户1不存在！'));
		}
		...
		$mum = $num;
		$peer = Db::name('UserCoin')->where(array('userid' => $addr_user['id']))->find();

		if (!$peer) {
			$this->error(lang('转入用户不存在！'));
		}
		try{
			$mo = Db::name();
			Db::execute('set autocommit=0');
			...
			$rs = array();
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $num);
			...
			// 转出人记录
			$rs[] = Db::table('tw_finance_log')->insert(array('username' => session('userName'), 'adminname' => session('userName'), 'addtime' => time(), 'plusminus' => 0, 'amount' => $num, 'optype' => 8, 'position' => 1, 'cointype' => $Coins['id'], 'old_amount' => $user_coin[$coin], 'new_amount' => $user_zj_coin[$coin], 'userid' => session('userId'), 'adminid' => session('userId'),'addip'=>get_client_ip()));

			// 接受人记录
			$rs[] = Db::table('tw_finance_log')->insert(array('username' => $user_info['username'], 'adminname' => session('userName'), 'addtime' => time(), 'plusminus' => 1, 'amount' => $mum, 'optype' => 9, 'position' => 1, 'cointype' => $Coins['id'], 'old_amount' => $peer[$coin], 'new_amount' => $user_peer_coin[$coin], 'userid' => $peer['userid'], 'adminid' => session('userId'),'addip'=>get_client_ip()));
	...
	public function mywt($market = NULL, $type = NULL, $status = NULL)
	{
		...
		// 获取币种信息
		$coin_info = Db::name('market')->where(array('name' => $market))->find();
		if(!$coin_info){
			$this->error(lang('币种不存在'));
		}
		$this->assign('coin_info', $coin_info);
		...
		$list = $Mobile->db(1,'DB_Read')->where($where)->order('id desc')->limit(0, 10)->select();
		foreach ($list as $k => $v) {
			$list[$k]['num'] = $v['num'] * 1;
