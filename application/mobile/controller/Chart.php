...
class Chart extends Mobile
{
	...
	protected function getTradeBuy($market)
	{
		...
		$mo = Db::name();
		$buy = Db::query('select id,price,sum(num-deal)as nums from movesay_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 100;');
		$data = '';
	...
	protected function getTradeSell($market)
	{
		...
		$mo = Db::name();
		$sell = Db::query('select id,price,sum(num-deal)as nums from movesay_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 100;');
		$data = '';
	...
	protected function getTradeLog($market)
	{
		...
		$log = Db::name('TradeLog')->where(array('status' => 1, 'market' => $market))->order('id desc')->limit(1000)->select();
		$data = '';
	...
	public function trend()
	{
		...
		$market = (is_array(config('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
	...
	public function getMarketSpecialtyJson()
	{
		...
		if (!$tradeJson) {
			...
			$tradeJson = Db::name('TradeJson')->where(array('type' => $time, 'market' => $market))->order('id desc')->limit(1000)->select();
				$sort = array(
	...
	public function getSpecialtyTrades()
	{
		...
		if (!$input['since']) {
			$tradeLog = Db::name('TradeLog')->where(array('market' => $input['market']))->order('id desc')->find();
