...
class Pay extends Mobile
{
	...
	public function index()
	{
		if ($this->request->isPost()) {
			...
			if (isset($_POST['alipay'])) {
				...
				Db::execute('lock tables tw_user_coin write,tw_mycz write,tw_finance write');
				$rs = array();
				$finance = Db::table('tw_finance')->where(array('userid' => $mycz['userid']))->order('id desc')->find();
				$finance_num_user_coin = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->find();
				$rs[] = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
				$rs[] = Db::table('tw_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
				$finance_mum_user_coin = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->find();
	...
	public function movepay()
	{
		if ($this->request->isPost()) {
			...
			$mo = Db::name();
			Db::execute('set autocommit=0');
			Db::execute('lock tables tw_user_coin write,tw_mycz write');
			$rs = array();
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = Db::table('tw_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function mycz($id = NULL)
	{
		...
		if (check($id, 'd')) {
			...
			$mycz = Db::name('Mycz')->where(array('tradeno' => $out_trade_no,'num' => $total_fee))->find();
			if (!$mycz) {
				$this->error('充值订单不存在！');
			}
			if ($mycz['status'] != 0) {
			...
			$rs = Db::name('Mycz')->where(array('id' => $mycz['id']))->save(array('status' => 3));
			if ($rs) {
				$res = $this->myczQueren($mycz['id']);
				if($res){
					Db::name('Mycz')->where(array('id'=>$mycz['id']))->save(array('ewmname'=>''));
					echo "success";
					exit;
				}
			}else {
	...
	private function myczQueren($id)
	{
		...
		if(flock($fp,LOCK_EX | LOCK_NB))
		{
			...
			$rs[] = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $ucoin);
			$rs[] = Db::table('tw_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['mum'], 'endtime' => time()));
			$finance_mum_user_coin = Db::table('tw_user_coin')->where(array('userid' => $mycz['userid']))->find();
