...
class AjaxtradeController extends MobileController
{
	...
	public function ajax_trade($coin_name = null)
	{
		...
		// 最新成交价格------------S
		$list_t = Db::name('TradeLog')->order('addtime desc')->find();
		$data['price_new'] = $list_t['price']*1;
		// 最新成交价格------------E

		// 最新一次成交价格--买一价------------S
		$list_buy = Db::name('TradeLog')->where(array('type'=>1))->order('addtime desc')->find();
		$data['price_buy_new'] = $list_buy['price']*1;
		// 最新一次成交价格--买一价------------E

		// 最新一次成交价格--卖一价------------S
		$list_sell = Db::name('TradeLog')->where(array('type'=>2))->order('addtime desc')->find();
		...
		$s_list = Db::name('Trade')->where($s_where)->order('price asc')->find();

		$data['buy_good_price'] = round($s_list['price'],3);

		$b_where = array('status'=>0,'type'=>1);
		$b_list = Db::name('Trade')->where($b_where)->order('price desc')->find();

		$data['sell_good_price'] = round($b_list['price'],3);
		// 处理买入价格与卖出价格（最佳买卖价）------------E

		// 获取涨跌比例------------S
		$bili = Db::name('Market')->where(array('name'=>$market))->find();
		...
		$list = Db::name('TradeLog')->where($where)->order('addtime desc')->find();
