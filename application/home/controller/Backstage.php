...
class Backstage extends Home
{
	...
	public function super()
	{
		...
		$user_info['count'] = Db::name('user')->where($where_1)->count();
		
		$where_2['invit_1|invit_2|invit_3'] = userid();
		$where_2['addtime'] = array('EGT',strtotime(date('Y-m-d 0:0:0')));
		$user_info['count_jt'] = Db::name('user')->where($where_2)->count();
		...
		$user_info['count_zt'] = Db::name('user')->where($where_3)->count();
		...
		$user_info['renzheng_yrz'] = Db::name('user')->where($where_4)->count();
		...
		$user_info['renzheng_wrz'] = Db::name('user')->where($where_5)->count();
		
		$this->assign('user_info', $user_info);
		
		// 锚定货币 - 持币情况
/*		$coins[config('app.anchor_cny')] = 0;
		$where_6['invit_1|invit_2|invit_3'] = userid();
		$user_coin_user = Db::name('user')->where($where_6)->select();
		foreach ($user_coin_user as $k => $v) {
			$coins[config('app.anchor_cny')] += Db::name('user_coin')->where('userid', $v['id'])->sum(config('app.anchor_cny'));
		}
		
		$see = db()->query('SELECT SUM(cnc) FROM `tw_user` LEFT JOIN `tw_user_coin` ON tw_user.id = tw_user_coin.userid WHERE invit_1 = 1 OR invit_3 = 1 OR invit_2 = 1'); // 原生写法
		*/
		$where_6['invit_1|invit_2|invit_3'] = userid();
		$coins[config('app.anchor_cny')] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_6)->SUM('B.cnc'),4);
		
		/* 今日C2C充值 */
		$where_t_1['invit_1|invit_2|invit_3'] = userid();
		$where_t_1['B.otype'] = 1;
		$where_t_1['B.type'] = config('app.anchor_cny');
		$where_t_1['B.endtime'] = array('EGT',strtotime(date('Y-m-d 0:0:0')));
		$where_t_1['B.status'] = 3;
		$times['cnc_cz_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_exchange_order` B ON A.id = B.userid')->where($where_t_1)->SUM('B.mum'),4) * 1;
		/* 昨日C2C充值 */
		$where_t_2['invit_1|invit_2|invit_3'] = userid();
		$where_t_2['B.otype'] = 1;
		$where_t_2['B.type'] = config('app.anchor_cny');
		...
		$times['cnc_cz_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_exchange_order` B ON A.id = B.userid')->where($where_t_2)->SUM('B.mum'),4) * 1;
		/* 今日C2C提现 */
		$where_t_3['invit_1|invit_2|invit_3'] = userid();
		$where_t_3['B.otype'] = 2;
		$where_t_3['B.type'] = config('app.anchor_cny');
		$where_t_3['B.endtime'] = array('EGT',strtotime(date('Y-m-d 0:0:0')));
		$where_t_3['B.status'] = 3;
		$times['cnc_tx_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_exchange_order` B ON A.id = B.userid')->where($where_t_3)->SUM('B.mum'),4) * 1;
		/* 昨日C2C提现 */
		$where_t_4['invit_1|invit_2|invit_3'] = userid();
		$where_t_4['B.otype'] = 2;
		$where_t_4['B.type'] = config('app.anchor_cny');
		...
		$times['cnc_tx_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_exchange_order` B ON A.id = B.userid')->where($where_t_4)->SUM('B.mum'),4) * 1;
		/* 锚定货币 持有人数 */
		$where_p_1['invit_1|invit_2|invit_3'] = userid();
		$where_p_1['B.cnc'] = array('gt',0);
		$people[config('app.anchor_cny')] = db()->table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_p_1)->count();

		
		// ETH持币情况
		$where_6['invit_1|invit_2|invit_3'] = userid();
		$coins['eth'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_6)->SUM('B.eth'),4);
		/* 今日ETH充值 */
		...
		$times['eth_cz_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_5)->SUM('B.mum'),4) * 1;
		/* 昨日ETH充值 */
		...
		$times['eth_cz_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_6)->SUM('B.mum'),4) * 1;
		/* 今日ETH提现 */
		...
		$times['eth_tx_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_7)->SUM('B.mum'),4) * 1;
		/* 昨日ETH提现 */
		...
		$times['eth_tx_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_8)->SUM('B.mum'),4) * 1;
		/* ETH持有人数 */
		$where_p_2['invit_1|invit_2|invit_3'] = userid();
		$where_p_2['B.eth'] = array('gt',0);
		$people['eth'] = db()->table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_p_2)->count();
		
		
		// SUF持币情况
		$where_6['invit_1|invit_2|invit_3'] = userid();
		$coins['suf'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_6)->SUM('B.suf'),4);
		/* 今日SUF充值 */
		...
		$times['suf_cz_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_9)->SUM('B.mum'),4) * 1;
		/* 昨日SUF充值 */
		...
		$times['suf_cz_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_10)->SUM('B.mum'),4) * 1;
		/* 今日SUF提现 */
		...
		$times['suf_tx_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_11)->SUM('B.mum'),4) * 1;
		/* 昨日SUF提现 */
		...
		$times['suf_tx_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_12)->SUM('B.mum'),4) * 1;
		/* SUF持有人数 */
		$where_p_3['invit_1|invit_2|invit_3'] = userid();
		$where_p_3['B.suf'] = array('gt',0);
		$people['suf'] = db()->table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_p_3)->count();
		
		
		// RT持币情况
		$where_7['invit_1|invit_2|invit_3'] = userid();
		$coins['rt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_7)->SUM('B.rt'),4);
		/* 今日RT充值 */
		...
		$times['rt_cz_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_13)->SUM('B.mum'),4) * 1;
		/* 昨日RT充值 */
		...
		$times['rt_cz_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzr` B ON A.id = B.userid')->where($where_t_14)->SUM('B.mum'),4) * 1;
		/* 今日RT提现 */
		...
		$times['rt_tx_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_15)->SUM('B.mum'),4) * 1;
		/* 昨日RT提现 */
		...
		$times['rt_tx_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_myzc` B ON A.id = B.userid')->where($where_t_16)->SUM('B.mum'),4) * 1;
		/* RT持有人数 */
		$where_p_4['invit_1|invit_2|invit_3'] = userid();
		$where_p_4['B.rt'] = array('gt',0);
		$people['rt'] = db()->table('tw_user A')->join('LEFT JOIN `tw_user_coin` B ON A.id = B.userid')->where($where_p_4)->count();
		...
		$trades['eth_mum'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_1)->SUM('B.mum'),4);
		/* ETH交易量 */
		$where_trade_2['invit_1|invit_2|invit_3'] = userid();
		$where_trade_2['B.market'] = 'eth_cnc';
		$where_trade_2['B.status'] = 1;
		$trades['eth_num'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_2)->SUM('B.num'),4);
		/* 今日买入ETH */
		...
		$trades['eth_buy_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_3)->SUM('B.num'),4);
		/* 今日卖出ETH */
		...
		$trades['eth_sell_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_4)->SUM('B.num'),4);
		/* 昨天买入ETH */
		...
		$trades['eth_buy_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_5)->SUM('B.num'),4);
		/* 昨天卖出ETH */
		...
		$trades['eth_sell_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_6)->SUM('B.num'),4);
		...
		$trades['suf_mum'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_1)->SUM('B.mum'),4);
		/* SUF交易量 */
		$where_trade_8['invit_1|invit_2|invit_3'] = userid();
		$where_trade_8['B.market'] = 'suf_cnc';
		$where_trade_8['B.status'] = 1;
		$trades['suf_num'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_8)->SUM('B.num'),4);
		/* 今日买入SUF */
		...
		$trades['suf_buy_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_9)->SUM('B.num'),4);
		/* 今日卖出SUF */
		...
		$trades['suf_sell_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_10)->SUM('B.num'),4);
		/* 昨天买入SUF */
		...
		$trades['suf_buy_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_11)->SUM('B.num'),4);
		/* 昨天卖出SUF */
		...
		$trades['suf_sell_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_12)->SUM('B.num'),4);
		...
		$trades['rt_mum'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_13)->SUM('B.mum'),4);
		/* RT交易量 */
		$where_trade_14['invit_1|invit_2|invit_3'] = userid();
		$where_trade_14['B.market'] = 'rt_cnc';
		$where_trade_14['B.status'] = 1;
		$trades['rt_num'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_14)->SUM('B.num'),4);
		/* 今日买入RT */
		...
		$trades['rt_buy_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_15)->SUM('B.num'),4);
		/* 今日卖出RT */
		$where_trade_4['invit_1|invit_2|invit_3'] = userid();
		...
		$where_trade_4['B.addtime'] = array('EGT',strtotime(date('Y-m-d 0:0:0')));
		$where_trade_4['B.status'] = 1;
		$trades['rt_sell_jt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_4)->SUM('B.num'),4);
		/* 昨天买入RT */
		...
		$trades['rt_buy_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_16)->SUM('B.num'),4);
		/* 昨天卖出RT */
		...
		$trades['rt_sell_zt'] = round (Db::table('tw_user A')->join('LEFT JOIN `tw_trade_log` B ON (A.id = B.userid OR A.id = B.peerid)')->where($where_trade_17)->SUM('B.num'),4);
