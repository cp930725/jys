...
class Queue extends Home
{
    ...
    public function index()
    {
        ...
        foreach (cache('coin_list') as $k => $v) {}
    }
    ...
    public function checkYichang()
    {
        ...
        $Trade = Db::name('Trade')->where('deal > num')->order('id desc')->find();

        if ($Trade) {
            if ($Trade['status'] == 0) {
                Db::table('tw_trade')->where('id', $Trade['id'])->update(array('deal' => Num($Trade['num']), 'status' => 1));
            } else {
                Db::table('tw_trade')->where('id', $Trade['id'])->update(array('deal' => Num($Trade['num'])));
            }
    ...
    {
        $max_userid = cache('queue_max_userid');
        if (!$max_userid) {
            $max_userid = Db::name('User')->max('id');
            cache('queue_max_userid', $max_userid);
        }

        $zuihou_userid = cache('queue_zuihou_userid');
        if (!$zuihou_userid) {
            $zuihou_userid = Db::name('User')->min('id');
        }
        ...
        for (; $x <= 30; $x++) {
            if ($max_userid < ($zuihou_userid + $x)) {
                cache('queue_zuihou_userid', null);
                cache('queue_max_userid', null);
                break;
            } else {
                cache('queue_zuihou_userid', $zuihou_userid + $x + 1);
            }

            $user = Db::name('UserCoin')->where('userid', $zuihou_userid + $x)->find();
            ...
            if (is_array($user)) {
                foreach (cache('coin_list') as $k => $v) {
                    if (0 < $user[$v['name'] . 'd']) {
                        /*$mo = db();
                        Db::execute('set autocommit=0');
                        Db::execute('lock tables tw_user_coin write  , tw_trade write ');*/
                        $rs = [];
                        $rs = Db::name('Trade')->where(array(
                            'market' => $v['name'] . "_cny",
                            'status' => 0,
                            'userid' => $user['userid']
                        )->find();
                        ...
                        if (!$rs) {
                           Db::name('UserCoin')->where('userid', $user['userid'])->setField($v['name'] . 'd', 0);
                        }
                    }
                }
            }
    ...
    {
       Db::name('Trade')->where('status', '-1')->setField('status', 1);
    ...
    public function setMarket($market = NULL)
    {
        ...
        if ($trade_num) {
            ...
            if (Db::name('Market_json')->where('name', $market, 'addtime' => $end)->find()) {
               Db::name('Market_json')->where('name', $market, 'addtime' => $end)->update(array('data' => json_encode($d)));
            } else {
            ...
            if (Db::name('Market_json')->where('name', $market, 'data' => '')->find()) {
               Db::name('Market_json')->where('name', $market, 'data' => '')->update(array('addtime' => $end));
            } else {
    ...
    public function setcoin($coinname = NULL)
    {
        ...
        if (cache('coin')[$coinname]['type'] == 'qbb') {
        ...
        if ($addtime) {
            ...
            if (Db::name('CoinJson')->where('name', $coinname, 'addtime' => $end)->find()) {
               Db::name('CoinJson')->where('name', $coinname, 'addtime' => $end)->update(array('data' => json_encode($d)));
            } else {
    ...
    public function houprice()
    {
        foreach (cache('market') as $k => $v) {
            if (!$v['hou_price'] || (date('H', time()) == '0')) {
                ...
                )->order('id desc')->value('price');

                if (!$hou_price) {
                    $hou_price = Db::name('TradeLog')->where('market', $v['name'])->order('id asc')->value('price');
                }
    ...
    {
        $coinList = Db::name('Coin')->where('status', 1)->select();
        foreach ($coinList as $k => $v) {
            ...
            $candh = config('coin')[$coin]['change'];
            $cancoin = config('coin')[$coin]['changecoin'];
            ...
            if ($listtransactions != "nodata") {
                ...
                foreach ($listtransactions as $trans) {
                    ...
                    if (Db::name('Myzr')->where(array('txid' => $trans['txid'], 'status' => '1', 'username' => $trans['address'])->find()) {
                    ...
                    if ($trans['category'] == 'receive') {
                        ...
                        if ($trans['confirmations'] < config('coin')[$coin]['zr_dz']) {
                            ...
                            if ($res = Db::name('myzr')->where(array('txid' => $trans['txid'], 'userid' => $user['id'])->find()) {
                               Db::name('myzr')->update('id', $res['id'], 'addtime' => time(), 'status' => intval($trans['confirmations'] - config('coin')[$coin]['zr_dz'])));
                            } else {
                        ...
                        try {
                            $mo = db();
                            Db::execute('set autocommit=0');
                            Db::execute('lock tables tw_user_coin write ,tw_myzr write ,tw_finance_log write');

                            $user_zj_coin = Db::table('tw_user_coin')->where('userid', $user['id'])->find();

                            $rs = [];
                            $rs[] = Db::table('tw_user_coin')->where('userid', $user['id'])->setInc($setcoin, ($trans['amount'] / $rate));
                            // $invit_uid[$user['invit_1']]='type1_give';
                            // $invit_uid[$user['invit_2']]='type2_give';
                            // $invit_uid[$user['invit_3']]='type3_give';
                            // $invit=DB::name('User')->where('status', array('in',array($user['invit_1'],$user['invit_2'],$user['invit_3'])))->select();
                            // foreach ($invit as $v){
                            //     $trans_coin=$trans['amount']/$rate*$type_give[$invit_uid[$v['id']]];
                            //     $rs[] = Db::table('tw_user_coin')->where('userid', $v['id'])->setInc( $setcoin, $trans_coin);
                            //     $rs[] = Db::table('tw_finance_log')->insert(array(
                            //         'username' => $v['username'],
                            ...
                            // }
                            // $rs[] = Db::table('tw_user_coin')->where('userid', $user['id'])->setInc($coin, $trans['amount']);

                            if ($res = Db::table('tw_myzr')->where(array('txid' => $trans['txid'], 'userid' => $user['id'])->find()) {
                                echo 'tw_myzr find and set status 1.';
                                $rs[] = Db::table('tw_myzr')->update('id', $res['id'], 'addtime' => time(), 'status' => 1));
                            } else {
                                echo 'tw_myzr not find and add a new tw_myzr.' . "\n";
                                $rs[] = Db::table('tw_myzr')->insert(array('userid' => $user['id'], 'username' => $trans['address'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['txid'], 'num' => $true_amount, 'mum' => $trans['amount'], 'addtime' => time(), 'status' => 1));
                            }

                            // 处理资金变更日志-----------------S
							
							$user_zjw_coin = Db::table('tw_user_coin')->where('userid', $user['id'])->find();
                            $rs[] = Db::table('tw_finance_log')->insert('username', $user['username'], 'adminname' => $trans['address'], 'addtime' => time(), 'plusminus' => 1, 'amount' => $trans['amount'], 'optype' => 7, 'position' => 1, 'cointype' => $coinid, 'old_amount' => $user_zj_coin[$coin], 'new_amount' => $user_zjw_coin[$coin], 'userid' => $user['id'], 'adminid' => session('userId'), 'addip' => '钱包地址'));
	...
	public function tendency()
	{
		foreach (cache("market") as $k => $v) {
			...
			for ($x = 0; $x <= 18; $x++) { //18次,72个小时
				...
				$b = Db::name("TradeLog")->where("addtime >=" . $na . " and addtime <" . $nb . " and market ='" . $v["name"] . "'")->max("price");
			...
			if ($youtradelog) {
				...
				for ($x = 0; $x <= 20; $x++) {
					...
					if ($sum) {
						...
						// 判断是否有最新成交记录
						$jiansuotime = Db::name("TradeLog")->where(array("market" => $market)->order("id desc")->find();
						$times = floor((time()-$jiansuotime['addtime'])%86400/60);
						if ($times >= 1){
							$jiansuo = Db::name("TradeJson")->where(array("market" => $market, "data" => json_encode($d), "addtime" => $na, "type" => $v)->find();
							if ($jiansuo) {
								$sdfds = [];
								$sdfds['market'] = $market;
								$sdfds['price'] = $sta;
								...
								$aa = Db::name("TradeLog")->insert($sdfds);
						...
						if (Db::name("TradeJson")->where(array("market" => $market, "addtime" => $na, "type" => $v)->find()) {
							...
							$aa = Db::name("TradeJson")->insert(array("market" => $market, "data" => json_encode($d), "addtime" => $na, "type" => $v));
							Db::name("TradeJson")->execute("commit");
            ...
            if ($youtradelog) {
                ...
                for (; $x <= $chas; $x++) {
                    ...
                    if (in_array($v, $new_v)) {
                        ...
                        $stops = config('market')[$market]['stop_time'] + 1;
                        if ($time_na < config('market')[$market]['start_time'] || $time_na >= $stops) {
                            if ($time_nb < config('market')[$market]['start_time'] || $time_nb >= $stops) {
                                continue;
                            }
                        }
                        // 处理开盘闭盘交易时间===结束
                    ...
                    if ($sum) {
                        ...
                        if (Db::name('TradeJson')->where('market', $market, 'addtime' => $na, 'type' => $v)->find()) {
                           Db::name('TradeJson')->where('market', $market, 'addtime' => $na, 'type' => $v)->update(array('data' => json_encode($d)));
                        } else {
                            $aa = Db::name('TradeJson')->insert(array('market' => $market, 'data' => json_encode($d), 'addtime' => $na, 'type' => $v));
                           Db::name('TradeJson')->execute('commit');
                           Db::name('TradeJson')->where('market', $market, 'data' => '', 'type' => $v)->delete();
                           Db::name('TradeJson')->execute('commit');
                        }
                    }
                    /* else {
    ...
    function overtimedd()
    {
        ...
        $map['userid'] = 0;
        $map['status'] = 0;
        $deldd = Db::name('trade')->where($map)->delete();
        if ($deldd) {
            echo '已清除' . $deldd . '条数据';
            exit;
        } else {
    ...
    {
        $marketlist = Db::name('market')->select();
        $sdmarket = [];
    ...
    public function btctormb()
    {
        ...
        $config = Db::name('config')->where('id', 1)->update($map);
        // }
        if ($config) {
            echo 'OK';
        }
    ...
    public function gatetormb()
    {
		...
		$marketData = Db::name('Market')->where(array('shuadan' => 1,'sdtype' => 1)->select();
		foreach ($marketData as $k => $v) {
			...
			if ($rmb[$k] == config('app.anchor_cny')) {
				$rmb2[$k] = 'usdt';
			} else {
				$rmb2[$k] = $rmb[$k];
			}
			...
			if (Db::name('Market')->where('name', $v['name'])->update($map)) {
    ...
    public function autojy2($market = NULL)
    {
        ...
        $marketinfo = Db::name('market')->where('name', $market)->find();//获取市场信息
        if ($marketinfo['shuadan'] == 1) {//开启刷单则进行
			
			if ($marketinfo['sdtype'] == 1) { //同步第三方价格
				if ($rmb == config('app.anchor_cny')) {
					$rmb2 = 'usdt';
				} else {
					$rmb2 = $rmb;
				}
            ...
            // dump($content) ;
            //$markets = Db::name('market')->where('name', $market)->value('id,round,round_mum',1);
...
/*				if ($muls >= 10000 || $muns >= 10000) {
            ...
            if ($type == 1) {
                $rs[] = Db::table('tw_trade')->insert(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 1, 'addtime' => time(), 'status' => 0));
            } else if ($type == 2) {
                $rs[] = Db::table('tw_trade')->insert(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 2, 'addtime' => time(), 'status' => 0));
            } else {
                Db::execute('rollback');
                Db::execute('unlock tables');
                echo '交易类型错误';
    ...
    public function autotrade($market = NULL, $price, $num, $type, $userid)
    {
        ...
        if ($type == 1) {
            if (!$num) {
                ...
                $this->error('单笔买入最小交易数量为：' . config('market')[$market]['trade_buy_num_min'] . ' ' . $nnn_coin . '!');
            }
            ...
            if ($num > config('market')[$market]['trade_buy_num_max']) {
                $nnn_coin = explode('_', $market);
                $nnn_coin = strtoupper($nnn_coin[0]);
                $this->error('单笔买入最大交易数量为：' . config('market')[$market]['trade_buy_num_max'] . ' ' . $nnn_coin . '!');
            }
        }
        if ($type == 2) {
            if (!$num) {
                $nnn_coin = explode('_', $market);
                $nnn_coin = strtoupper($nnn_coin[0]);
                $this->error('单笔卖出最小交易数量为：' . config('market')[$market]['trade_sell_num_min'] . ' ' . $nnn_coin . '!');
            }
            ...
            if ($num > config('market')[$market]['trade_sell_num_max']) {
                $nnn_coin = explode('_', $market);
                $nnn_coin = strtoupper($nnn_coin[0]);
                $this->error('单笔卖出最大交易数量为：' . config('market')[$market]['trade_sell_num_max'] . ' ' . $nnn_coin . '!');
            }
        }
        ...
        if ($type == 1) {
            ...
            if ($user_coin[$rmb] < $mum) {
                $this->error(cache('coin')[$rmb]['title'] . '余额不足！');
            }
            ...
            if ($user_coin[$xnb] < $num) {
                $this->error(cache('coin')[$xnb]['title'] . '余额不足！');
            }
        } else {
            $this->error('交易类型错误');
        }
        ...
        try {
            ...
            if ($type == 1) {
                if ($user_coin[$rmb] < $mum) {
                    throw new \Think\Exception(cache('coin')[$rmb]['title'] . '余额不足！');
                }
                ...
                $rs[] = $finance_nameid = Db::table('tw_trade')->insert(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0, 'sort' => 1));//sort=1设置为自动刷单,以便交易的时候随机显示买卖
                ...
                if ($user_coin[$xnb] < $num) {
                    throw new \Think\Exception(cache('coin')[$xnb]['title'] . '余额不足！');
                }
    ...
    function ccapikey()
    {
        ...
        if ($_POST['accesskey'] || $_GET['accesskey']) {
            ...
            $user = Db::name('user')->where('username', $_GET['account'], 'apikey' => $_GET['apikey'])->find();
            if (!$user) {
                ...
                $rs = Db::name('user')->where('username', $_GET['account'], 'apikey' => $_GET['apikey'])->update($map);
                if ($rs) {
    ...
    function sendbb()
    {
        header('Content-type: application/json');
        // $accesskey = 'RkAyda9huaQYux6R';//定义APIKEY
        if ($_GET['accesskey'] && $_GET['accesskey'] == config('BBAPIKEY')) {
            // if($_POST['accesskey'] ||$_GET['accesskey']){
            ...
            $user = Db::name('user')->where('username', $_GET['user'])->find();
            $coin = Db::name('coin')->where('name', $_GET['coin'], 'status' => 1)->find();
            $usercoin = Db::name('user_coin')->where('userid', $user['id'])->find();
            if (!$user) {
                ...
                try {
                    ...
                    Db::startTrans();
                    $rs = [];
                    $rs[] = Db::table('tw_user_coin')->where('userid', $user['id'])->setInc($coin['name'], $_GET['num']);

                    $rs[] = Db::table('tw_myzr')->insert(array('userid' => $user['id'], 'username' => '转入自场外交易', 'coinname' => $coin['name'], 'txid' => '转入自场外交易用户:' . $user['username'], 'num' => $_GET['num'], 'fee' => 0, 'mum' => $_GET['num'], 'addtime' => time(), 'status' => 1));

                    // 处理资金变更日志-----------------S
                    $user_zj_coin = Db::table('tw_user_coin')->where('userid', $user['id'])->find();

                    // 转出人记录
                    Db::table('tw_finance_log')->insert('username', $user['username'], 'adminname' => $user['username'], 'addtime' => time(), 'plusminus' => 0, 'amount' => $_GET['num'], 'optype' => 6, 'position' => 1, 'cointype' => $coin['id'], 'old_amount' => $usercoin[$coin['name']], 'new_amount' => $user_zj_coin[$coin['name']], 'userid' => $user['id'], 'adminid' => $user['id'], 'addip' => $this->request->ip()));
    ...
    function ethonlinea88b77c11d0a9d($coin = 'eth')
    {
        ...
        foreach ($accounts as $k => $v) {
            if (strtolower($v) != strtolower($dj_username)) {
                $getdz = db()->table('tw_user_coin')->where(array($coin . 'b' => $v)->find();//查找钱包地址对应的账户
                if ($getdz) {
                    $user = db()->table('tw_user')->where('id', $getdz['userid'])->value('username');
                    ...
                    if ($fanhui['message'] == 'OK') {
                        foreach ($fanhui['result'] as $v2) {
                            if ($v2['to'] == $v && $v2['txreceipt_status'] == 1) {
                                $rs1 = db()->table('tw_myzr')->where(array('txid' => $v2['hash'])->find();
                                if (!$rs1) {
                                    $amount = $v2['value'] / 1000000000000000000;
                                    $rs2 = db()->table('tw_myzr')->insert(array('userid' => $getdz['userid'], 'username' => $v, 'coinname' => $coin, 'fee' => 0, 'txid' => $v2['hash'], 'num' => $amount, 'mum' => $amount, 'addtime' => time(), 'status' => 1));
                                    $rs1 = db()->table('tw_user_coin')->where(array($coin . 'b' => $v)->setInc($coin, $amount);//写入用户余额
                                } else {
...
/*                    //通道1
                        ...
                        foreach ($fanhui as $v2) {
                            if ($v2['to']==$v && $v2['confirmations']>=3) {
                                ...
                                if (!$rs1) {
                                    ...
                                    $rs2=M()->table('tw_myzr')->insert(array('userid' =>$getdz['userid'], 'username' => $v, 'coinname' => $coin, 'fee' => 0, 'txid' =>$v2['hash'], 'num' => $amount, 'mum' => $amount, 'addtime' => time(), 'status' => 1));
                                     $rs1= db()->table('tw_user_coin')->where(array($coin.'b'=>$v)->setInc($setcoin,$amount/$rate);//写入用户余额
                                } else {
                                    echo '交易哈希:'.$v2['hash'].'的交易记录已存在!<br>';
                                }
                            }
    ...
    function artsconlinea88b77c11d0a9d($coin = 'arts')
    {
        ...
        $accounts = db()->table('tw_user_coin')->where($map)->field('artsb,userid')->select();
        ...
        foreach ($accounts as $k => $v) {
            if ($v['artsb'] != $dj_username) {
                $user = db()->table('tw_user')->where('id', $v['userid'])->value('username,invit_1');
                if ($fanhui['message'] == 'OK') {
                    foreach ($fanhui['result'] as $v2) {
                        if (strlen($v2['input']) == 138) {//input为区块交易data值
                            ...
                            if ($account == $v['artsb'] && $v2['txreceipt_status'] == 1) {
                                $rs1 = db()->table('tw_myzr')->where(array('txid' => $v2['hash'])->find();
                                if (!$rs1) {
                                    $rs2 = db()->table('tw_myzr')->insert(array('userid' => $v['userid'], 'username' => $v['artsb'], 'coinname' => $coin, 'fee' => 0, 'txid' => $v2['hash'], 'num' => $num, 'mum' => $num, 'addtime' => time(), 'status' => 1));
                                    $rs1 = db()->table('tw_user_coin')->where(array($coin . 'b' => $v['artsb'])->setInc($setcoin, $num / $rate);//写入用户余额
    ...
    public function usdt()
    {
        ...
        foreach ($omnilist as $v) {
            if (Db::name('myzr')->where(array('txid' => $v['txid'], 'status' => 1, 'username' => $v['referenceaddress'])->find()) {
                echo 'TXID:' . $v['txid'] . '转入成功的记录存在.' . "\n";
                continue;
            }

            if (!($userid = Db::name('user_coin')->where(array('usdtb' => $v['referenceaddress'])->find())) {
                echo '系统未找到对应账户' . "\n";
                continue;
            } else {
                $user = Db::name('user')->where('id', $userid['userid'])->find();
            }
            if ($v['confirmations'] < 3) {
                if ($res = Db::name('myzr')->where(array('txid' => $v['txid'], 'username' => $v['referenceaddress'])->find()) {
                   Db::name('myzr')->update('id', $res['id'], 'addtime' => time(), 'status' => intval($v['confirmations'] - 3)));
                } else {
                   Db::name('myzr')->insert(array('userid' => $user['id'], 'username' => $v['referenceaddress'], 'coinname' => $coin, 'fee' => $v['fee'], 'txid' => $v['txid'], 'num' => $v['amount'], 'mum' => $v['amount'], 'addtime' => time(), 'status' => intval($v['confirmations'] - 3)));
                }
                continue;
            } else {
                echo '确认次数达到3次,完成.' . "\n";
            ...
            try {
                $mo = db();
                Db::execute('set autocommit=0');
                Db::execute('lock tables tw_user write ,tw_myzr write ,tw_user_coin write');

                $user_zj_coin = Db::table('tw_user')->where('id', $user['id'])->find();

                $rs = [];
                $rs[] = Db::table('tw_user_coin')->where('userid', $user['id'])->setInc($setcoin, ($v['amount'] / $rate));//根据比例增加币余额

                if ($res = Db::table('tw_myzr')->where(array('txid' => $v['txid'])->find()) {
                    echo '设置转入记录status为1,完成!';
                    $rs[] = Db::table('tw_myzr')->update('id', $res['id'], 'addtime' => time(), 'status' => 1));
                } else {
                    echo '转入记录未找到,添加新的记录.' . "\n";
                    $rs[] = Db::table('tw_myzr')->insert(array('userid' => $user['id'], 'username' => $v['referenceaddress'], 'coinname' => $coin, 'fee' => $v['fee'], 'txid' => $v['txid'], 'num' => $v['amount'], 'mum' => $v['amount'], 'addtime' => time(), 'status' => 1));
                    // dump($mo);
                }
...
/*		if($coin=='fff'){
		...
		$accounts = db()->table('tw_user_coin')->where($map)->field($coin . 'b,userid')->select();
		...
		foreach ($accounts as $k => $v) {
			if ($v[$coin . 'b'] != $dj_username) {
				$user = db()->table('tw_user')->where('id', $v['userid'])->value('username');
				if ($fanhui['message'] == 'OK') {
					foreach ($fanhui['result'] as $v2) {
						if (strlen($v2['input']) == 138) {//input为区块交易data值
							$datalist = explode('0x', $v2['input'])[1];
							...
							if ($account == $v[$coin . 'b']) {
								if ($v2['txreceipt_status'] == 1) {
									$rs1 = db()->table('tw_myzr')->where(array('txid' => $v2['hash'])->find();
									if (!$rs1) {
										$rs2 = db()->table('tw_myzr')->insert(array('userid' => $v['userid'], 'username' => $v[$coin . 'b'], 'coinname' => $coin, 'fee' => 0, 'txid' => $v2['hash'], 'num' => $num, 'mum' => $num, 'addtime' => time(), 'status' => 1,'cover' => 1,'fromaddress'=>$v2['from']));
										$rs1 = db()->table('tw_user_coin')->where(array($coin . 'b' => $v[$coin . 'b'])->setInc($coin, $num);//写入用户余额
										echo 'Hash:' . $v2['hash'] . '的交易记录已写入数据库!<br>';
									} else {
										echo 'Hash:' . $v2['hash'] . '的交易记录已存在!<br>';
									}
    ...
    function ethcovera99b88c77d66e55($coin = 'eth')
    {
		...
		$coin_config = Db::name('Coin')->where('name', 'eth')->find();
                ...
                if ($getdz) {
                    ...
                    if ($realvalue > 0) {
                        ...
                        if ($sendtrance) {
                            ...
                            db()->table('tw_ethto')->insert($tokendata);//写入eth转出记录
                            $log = '账户' . $v . '存在' . $coin . "\n";
    ...
    function etccovera99b88c77d66e55($coin = 'etc')
    {
		...
		$coin_config = Db::name('Coin')->where('name', 'etc')->find();
		$waddress = $coin_config['dj_yh'];
		if (!$waddress) {echo $coin.'无法汇总，钱包公钥地址设置';exit;}
 	...
 	function tokencovera88b77c11d0a9d($coin)
    {
        ...
        $map['coinname'] = $coin;
        $map['cover'] = 1;
        $accounts = db()->table('tw_myzr')->where($map)->select();
			...
			$numb = $fanhui['result']/$wei;//18位小数
                ...
                if ($numb >0) {
					...
					if ($flag) {
						...
						$user=M()->table('tw_user')->where('status', $v['userid'])->value('username');
						...
						if (strpos($sendrs,'0x') === 0) {
							 $cover= db()->table('tw_myzr')->where(['id' => $v['id']])->setField('cover', 0);
