...
class IssueController extends HomeController
{
	...
	public function index()
	{
		...
		$show = $Page->show();
		//$list = $Model->fetchSql()->where($where)->order('addtime desc')->limit(0, 10)->select();

		$list = $Model->where($where)->order('tuijian asc,paixu desc,addtime desc')->limit(0, 10)->select();
		...
		foreach ($list as $k => $v) {
			//$list[$k]['img'] = Db::name('Coin')->where('name', $v['coinname'])->value('img');
	...
	public function buy($id=1)
	{
		...
		// }
		if($id==1){
			$id=DB::name('Issue')->order('id asc')->limit(1)->value('id');
		}
		...
		$Issue['img'] = Db::name('Coin')->where('name', $Issue['coinname'])->value('img');
		// echo ($Issue['price']);
	...
	public function upbuy($id, $num, $paypassword)
	{
		...
		*/
		...
		$issueLog = Db::name('IssueLog')->where('userid', userid(), 'coinname' => $Issue['coinname'])->sum('num');
		...
		Db::execute('lock tables tw_invit write ,  tw_user_coin write  , tw_issue write  , tw_issue_log  write ,tw_finance write');
		...
		$rs[] = $finance_nameid = Db::table('tw_issue_log')->insert(array('userid' => userid(), 'coinname' => $Issue['coinname'], 'buycoin' => $Issue['buycoin'], 'name' => $Issue['name'], 'price' => $Issue['price'], 'num' => $num, 'mum' => $mum, 'ci' => $Issue['ci'], 'jian' => $Issue['jian'], 'unlock' => 0, 'addtime' => time(), 'endtime' => ($Issue['endtime']+ (60 * 60 * $Issue['jian'])), 'status' => 0));
	...
	public function unlock($id)
	{
		...
		$rs[] = Db::table('tw_issue_log')->where('id', $IssueLog['id'])->update(array('unlock' => $IssueLog['unlock'] + 1, 'endtime' => $tm));//下次解冻时间endtime修改成+间隔
		// $rs[] = Db::table('tw_issue_log')->where('id', $IssueLog['id'])->update(array('unlock' => $IssueLog['unlock'] + 1, 'endtime' => time()));

		if ($IssueLog['ci'] <= $IssueLog['unlock'] + 1) {
			$rs[] = Db::table('tw_issue_log')->where('id', $IssueLog['id'])->update(array('status' => 1));
		}
