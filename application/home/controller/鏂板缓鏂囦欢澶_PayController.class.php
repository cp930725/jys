<?php
namespace Home\Controller;

class PayController extends HomeController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","movepay","mycz","myczQueren","ecpss");
		if(!in_array($this->request->action(),$allow_action)){
			$this->error("非法操作！");
		}
	}

	public function index()
	{
		if ($this->request->ispost()) {
			foreach ($_POST as $k => $v) {
				// 过滤非法字符----------------S
				if (checkstr($v)) {
					$this->error(lang('您输入的信息有误！'));
				}
				// 过滤非法字符----------------E
			}

			if (isset($_POST['alipay'])) {
				$arr = explode('--', $_POST['alipay']);

				if (md5('movesay') != $arr[2]) {
					echo -1;
					exit();
				}

				if (!strstr($arr[0], 'Pay')) {
				}

				$arr[0] = trim(str_replace(PHP_EOL, '', $arr[0]));
				$arr[1] = trim(str_replace(PHP_EOL, '', $arr[1]));

				if (strstr($arr[0], '付款-')) {
					$arr[0] = str_replace('付款-', '', $arr[0]);
				}

				$mycz = Db::name('Mycz')->where('tradeno', $arr[0])->find();

				if (!$mycz) {
					echo -3;
					exit();
				}

				if (($mycz['status'] != 0) && ($mycz['status'] != 3)) {
					echo -4;
					exit();
				}

				if ($mycz['num'] != $arr[1]) {
					echo -5;
					exit();
				}

				$mo = db();
				Db::execute('set autocommit=0');
				Db::execute('lock tables tw_user_coin write,tw_mycz write,tw_finance write');
				$rs = [];
				$finance = Db::table('tw_finance')->where('userid', $mycz['userid'])->order('id desc')->find();
				$finance_num_user_coin = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->find();
				$rs[] = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->setInc('cny', $mycz['num']);
				$rs[] = Db::table('tw_mycz')->where('id', $mycz['id'])->update(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
				$finance_mum_user_coin = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->find();
				$finance_hash = md5($mycz['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mycz['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'tp3.net.cn');
				$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];

				if ($finance['mum'] < $finance_num) {
					$finance_status = (1 < ($finance_num - $finance['mum']) ? 0 : 1);
				}
				else {
					$finance_status = (1 < ($finance['mum'] - $finance_num) ? 0 : 1);
				}

				$rs[] = Db::table('tw_finance')->insert(array('userid' => $mycz['userid'], 'coinname' => 'cny', 'num_a' => $finance_num_user_coin['cny'], 'num_b' => $finance_num_user_coin['cnyd'], 'num' => $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'], 'fee' => $mycz['num'], 'type' => 1, 'name' => 'mycz', 'nameid' => $mycz['id'], 'remark' => '人民币充值-人工到账', 'mum_a' => $finance_mum_user_coin['cny'], 'mum_b' => $finance_mum_user_coin['cnyd'], 'mum' => $finance_mum_user_coin['cny'] + $finance_mum_user_coin['cnyd'], 'move' => $finance_hash, 'addtime' => time(), 'status' => $finance_status));

				if (check_arr($rs)) {
					Db::execute('commit');
					Db::execute('unlock tables');
					echo 1;
					exit();
				}
				else {
					Db::execute('rollback');
					Db::query('rollback');
					echo -6;
					exit();
				}
			}
		}
	}

	public function movepay()
	{
		if ($this->request->ispost()) {


			foreach ($_POST as $k => $v) {
				// 过滤非法字符----------------S

				if (checkstr($v)) {
					$this->error('您输入的信息有误！');
				}

				// 过滤非法字符----------------E
			}


			$movepay = $_POST['movepay'];
			$tradeno = $_POST['tradeno'];
			$num = $_POST['num'];
			$status = $_POST['status'];

			if (md5('movesay') != $movepay) {
				echo -1;
				exit();
			}

			$mycz = Db::name('Mycz')->where('tradeno', $tradeno)->find();

			if (!$mycz) {
				echo -2;
				exit();
			}

			if ($mycz['status']) {
				echo -3;
				exit();
			}

			if ($mycz['num'] != $num) {
				echo -4;
				exit();
			}

			$mo = db();
			Db::execute('set autocommit=0');
			Db::execute('lock tables tw_user_coin write,tw_mycz write');
			$rs = [];
			$rs[] = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->setInc('cny', $mycz['num']);
			$rs[] = Db::table('tw_mycz')->where('id', $mycz['id'])->update(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));

			if (check_arr($rs)) {
				Db::execute('commit');
				Db::execute('unlock tables');
				$this->redirect('Mycz/log');
				exit();
			}
			else {
				Db::execute('rollback');
				Db::query('rollback');
				echo -5;
				exit();
			}
		}
	}

	public function mycz($id = NULL)
	{


		// 过滤非法字符----------------S

		if (checkstr($id)) {
			$this->error('您输入的信息有误！');
		}

		// 过滤非法字符----------------E


		if (check($id, 'd')) {
			$mycz = Db::name('Mycz')->where('id', $id)->find();

			if (!$mycz) {
				$this->redirect('Finance/mycz');
			}

			$myczType = Db::name('MyczType')->where('name', $mycz['type'])->find();

			if ($mycz['type'] == 'bank') {
				$UserBankType = Db::name('UserBankType')->where('status', 1)->order('id desc')->select();
				$this->assign('UserBankType', $UserBankType);
			}

			$this->assign('myczType', $myczType);
			$this->assign('mycz', $mycz);
			$this->display($mycz['type']);
		}
		else {
			Vendor("Pay.JSAPI","",".php");
			$wxpay_res=new \WxPayApi;
			$results = $wxpay_res->notify();
			$out_trade_no=$results['out_trade_no'];
			$total_fee=$results['total_fee']/100;

			$mycz = Db::name('Mycz')->where('tradeno', $out_trade_no,'num' => $total_fee)->find();
			if (!$mycz) {
				$this->error('充值订单不存在！');
			}
			if ($mycz['status'] != 0) {
				$this->error('订单已经处理过！');
			}
			$rs = Db::name('Mycz')->where('id', $mycz['id'])->update(array('status' => 3));
			if ($rs) {
				$res = $this->myczQueren($mycz['id']);
				if($res){
					Db::name('Mycz')->where('status', $mycz['id'])->update(array('ewmname'=>''));
					echo "success";
					exit;
				}
			}else {
				$this->error('订单更新失败！');
			}
		}
	}

	private function myczQueren($id)
	{


		// 过滤非法字符----------------S

		if (checkstr($id)) {
			$this->error('您输入的信息有误！');
		}

		// 过滤非法字符----------------E


		if (empty($id)) {
			$this->error('缺少参数!');
		}

		$mycz = Db::name('Mycz')->where('id', $id)->find();

		if (($mycz['status'] != 0) && ($mycz['status'] != 3)) {
			$this->error('已经处理，禁止再次操作！');
		}
		$fp = fopen("C:\inetpub\wwwroot\thehuay\lockcz.txt", "w+");
		if(flock($fp,LOCK_EX | LOCK_NB))
		{
			$mo = db();
			Db::execute('set autocommit=0');
			Db::execute('lock tables tw_user_coin write,tw_mycz write,tw_finance write');
			$rs = [];
			$finance = Db::table('tw_finance')->where('userid', $mycz['userid'])->order('id desc')->find();
			$finance_num_user_coin = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->find();
			$ucoin = $mycz['num']*(1-$mycz['fee']/100);
			$rs[] = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->setInc('cny', $ucoin);
			$rs[] = Db::table('tw_mycz')->where('id', $mycz['id'])->update(array('status' => 1, 'mum' => $mycz['mum'], 'endtime' => time()));
			$finance_mum_user_coin = Db::table('tw_user_coin')->where('userid', $mycz['userid'])->find();
			$finance_hash = md5($mycz['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mycz['mum'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE);
			$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];

			if ($finance['mum'] < $finance_num) {
				$finance_status = (1 < ($finance_num - $finance['mum']) ? 0 : 1);
			}
			else {
				$finance_status = (1 < ($finance['mum'] - $finance_num) ? 0 : 1);
			}

			$rs[] = Db::table('tw_finance')->insert(array('userid' => $mycz['userid'], 'coinname' => 'cny', 'num_a' => $finance_num_user_coin['cny'], 'num_b' => $finance_num_user_coin['cnyd'], 'num' => $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'], 'fee' => $mycz['num'], 'type' => 1, 'name' => 'mycz', 'nameid' => $mycz['id'], 'remark' => '人民币充值-自动充值', 'mum_a' => $finance_mum_user_coin['cny'], 'mum_b' => $finance_mum_user_coin['cnyd'], 'mum' => $finance_mum_user_coin['cny'] + $finance_mum_user_coin['cnyd'], 'move' => $finance_hash, 'addtime' => time(), 'status' => $finance_status));

			if (check_arr($rs)) {
				Db::execute('commit');
				Db::execute('unlock tables');
				$message="操作成功";
				$res=1;
			}
			else {
				Db::execute('rollback');
				$message="操作失败";
				$res=0;
			}
			flock($fp,LOCK_UN);
		}else{
			$message="请稍后提交";
			$res=0;
		}
		fclose($fp);
		if($res==1){
			return true;
		}else{
			return false;
		}
	}

	public function ecpss($id = NULL)
	{

		// 过滤非法字符----------------S

		if (checkstr($id)) {
			$this->error('您输入的信息有误！');
		}

		// 过滤非法字符----------------E


		if (!userid()) {
			$this->error('请先登录！');
		}

		if (empty($id)) {
			$this->error('参数错误！');
		}

		$mycz = Db::name('Mycz')->where('id', $id)->find();

		if (!$mycz) {
			$this->error('订单不存在！');
		}

		if ($mycz['userid'] != userid()) {
			$this->error('参数非法！');
		}

		$this->error('订单不存在！');
	}
}

?>