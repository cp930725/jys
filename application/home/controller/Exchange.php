...
class Exchange extends Home
{
	...
	public function index()
	{
		...
		if (!userid()) {
			$this->error(lang('请先登录！'),url('Login/index'));
		}
		...
		// 搜索银行卡信息
		$user_bank = Db::name('user_bank')->where('userid', userid())->find();
		if (!($user_bank)) {
			$this->assign('banks', 0);
		} else {
			$this->assign('banks', 1);
		...
		$order_info = Db::name('exchange_order')->where('userid', userid())->where('otype', 1)->where('status', 1)->find();
		$order_agent_info = Db::name('exchange_agent')->where('id', $order_info['aid'])->find();
		$this->assign('order_info', $order_info);
		$this->assign('order_agent_info', $order_agent_info);
		
		$UserCoin = Db::name('user_coin')->where('userid', userid())->find();
		
		$cny['ky'] = round($UserCoin[config('app.anchor_cny')], 2) * 1;
	...
	public function upTrade($price, $num, $otype)
	{
		...
		$coin_type = config('app.anchor_cny'); //充值类型
		
		if (Db::name('exchange_order')->where('userid', userid())->where('otype', 1)->where('status', 1)->find()) {
			$this->error(lang('您有订单未完成，无法创建！'));
		}
		
		/** 检查设置条件 **/
		$configs = Db::name('exchange_config')->where('id', 1)->find();
		if($configs['mycz_status'] == 0){
			$this->error(lang('网络繁忙，请稍后再试！'));
		}
		
		$price = $configs['mycz_uprice'];
		
		if ($num < $configs['mycz_min']) {
			$this->error(lang('每次充值金额不能小于') . $configs['mycz_min'] . lang('元！'));
		}
		if ($configs['mycz_max'] < $num) {
			$this->error(lang('每次充值金额不能大于') . $configs['mycz_max'] . lang('元！'));
		}
		...
		/** 随机匹配代理商 **/
		$data_agent = Db::name('exchange_agent')->where('status', 1)->field("id,aid")->limit(1)->order('rand()')->find();
		
		/** 实际到账金额 **/
		$mum = $num * $price;
		$my_order_no = build_order_no();
		$mycz = Db::name('exchange_order')->insert(array('otype' => $otype, 'orderid' => $my_order_no, 'userid' => userid(),'remarks' => $tradeno, 'uprice' => $price, 'num' => $num, 'mum' => $mum, 'type' => $coin_type, 'aid' => $data_agent['id'], 'addtime' => time(), 'status' => 1));
		
		if ($mycz) {
//商户标志，Huobi Pay后台获取
$channelSign = 'm9ThUHNB37hfJBHR';
//买家姓名，接入方自行设定
...
$orderNo = $my_order_no;
//跳转地址，交易完成跳转，接入方自行设定
$pickupUrl = 'http://www.hdzcrlzy.com/finance/index.html';
//回调地址，接受支付结果，接入方自行设定
$receiveUrl = 'http://www.hdzcrlzy.com/Exchange/notify.html';
//签名类型，固定值：MD5
$signType = 'MD5';
//商户秘钥，Huobi Pay后台获取
$md5key = '234578268735628';
//提交支付URL
$payUrl = 'http://api.huobipay.hk/payment/' . $channelSign;
$data = array(
...
//签名
$sign = md5($data['pickupUrl'] . $data['receiveUrl'] . $data['signType'] . $data['orderNo'] . $data['orderAmount'] . $data['orderCurrency'] . $data['customerId'] . $md5key);
//拼装参数
$query = http_build_query($data);
$href = $payUrl . '?' . $query . '&sign=' . $sign;
			$this->success(lang('订单创建成功！'), ['id', $mycz,'aid' => $data_agent['id'],'href'=>$href]);
		} else {
	...
	{
		$md5key = '234578268735628';
		$sign = md5($_POST['signType'] . $_POST['orderNo'] . $_POST['orderAmount'] . $_POST['orderCurrency'] . $_POST['transactionId'] . $_POST['status'] . $md5key);
		if ($sign == $_POST['sign']) {
		    if ($_POST['status']=='success') {
		        $myOrder = Db::name('exchange_order')->where('orderid', $_POST['orderNo'])->where('otype', 1)->where('type', 'cnc')->limit(1)->find();
		        if ($myOrder && ($myOrder['status']==1 || $myOrder['status']==2)) {
		           Db::name('exchange_order')->where('id', $myOrder['id'])->update(array('status'=>3,'endtime'=>time()));
		           Db::table('tw_user_coin')->where('userid', $myOrder['userid'])->setInc('cnc',$myOrder['mum']);
		        }
		    } elseif ($_POST['status']=='close') {
		        $myOrder = Db::name('exchange_order')->where('orderid', $_POST['orderNo'])->where('otype', 1)->where('type', 'cnc')->limit(1)->find();
		        if ($myOrder && ($myOrder['status']==1 || $myOrder['status']==2)) {
		           Db::name('exchange_order')->where('id', $myOrder['id'])->update(array('status'=>8,'endtime'=>time()));
		        }
		    } else {
	...
	public function upMytx($price, $num, $otype)
	{
		...
		$coin_type = config('app.anchor_cny'); //提现类型
		$coin_type_id = 2;
		
		/** 帐号金额 **/
		$user_coin = Db::name('user_coin')->where('userid', userid())->find();
		if ($user_coin[$coin_type] < $num) {
			$this->error(cache('coin')[$coin_type]['title'] . lang('余额不足！'));
		}
		...
		if ($num < $configs['mytx_min']) {
			$this->error(lang('每次提现金额不能小于') . $configs['mytx_min'] . lang('元！'));
		}

		if ($configs['mytx_max'] < $num) {
			$this->error(lang('每次提现金额不能大于') . $configs['mytx_max'] . lang('元！'));
		}
		if ($configs['mytx_bei']) {
			if ($num % $configs['mytx_bei'] != 0) {
				$this->error(lang('每次提现金额必须是') . $configs['mytx_bei'] . lang('的整倍数！'));
			}
		}
		...
		try{
			...
			Db::execute('lock tables tw_exchange_order write , tw_user_coin write ,tw_finance write,tw_finance_log write');
			 ...
			 * 操作数据 amount
			 */
			$rs[] = Db::table('tw_finance_log')->insert(['username' => session('userName'), 'adminname' => session('userName'), 'addtime' => time(), 'plusminus' => 0, 'amount' => $num, 'optype' => 5, 'position' => 1, 'cointype' => config("coin")[$coin_type]["id"], 'old_amount' =>  $finance_num_user_coin[$coin_type], 'new_amount' => $finance_mum_user_coin[$coin_type], 'userid' => session('userId'), 'adminid' => session('userId'),'addip'=>$this->request->ip()]);
	...
	public function bank()
	{
		...
		if ($user['kyc_lv'] == 1) {
			if ($user['idstate'] == 2) {
				...
				$this->error(lang('请先通过实名认证，再进行操作！'), url('User/index'));
			}
		} else if ($user['kyc_lv'] == 2) {
			$this->assign('idcard', 1);
		}
...
/*			if ($v['name'] == $name) {
		...
		if (Db::name('UserBank')->insert(array('userid' => userid(), 'truename' => $truename, 'bank' => $bank, 'bankprov' => $bankprov, 'bankcity' => $bankcity, 'bankaddr' => $bankaddr, 'bankcard' => $bankcard, 'addtime' => time(), 'status' => 1))) {
