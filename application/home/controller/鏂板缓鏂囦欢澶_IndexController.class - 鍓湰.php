...
class IndexController extends HomeController
{
	...
	public function index()
	{
		...
		foreach (cache('market') as $k => $v) {
			...
			// 处理卖-价与买-价---------------------S
			$buy_qq = Db::name('Trade')->where(array('type' => 1, 'market' => $k, 'status' => 0)->max('price');
			if (empty($buy_qq)) {
				$bbb = Db::name('TradeLog')->where('market', $k, 'status' => 1)->order('addtime desc')->find();
				$buy_qq = $bbb['price'];
			}
			$sell_qq = Db::name('Trade')->where(array('type' => 2, 'market' => $k, 'status' => 0)->min('price');
			if (empty($sell_qq)) {
				$sss = Db::name('TradeLog')->where('market', $k, 'status' => 1)->order('addtime desc')->find();
				$sell_qq = $sss['price'];
			}
			// 处理卖-价与买-价---------------------E
